...
- 描述1：其中react_agent模式是我根据原langgraph的各流程节点复刻的，
- 原因：原langgrap的react_agent框架类似黑盒子，每一步节点怎么走无法由用户提示词准确控制。
- 描述2：用于存储对话的**数据状态**已经从langgraph版的state更换为自己设计的**字典state**，分为节点间传输state和对话存储state。
- 为什么要改state？
- 原因：
- langgraph的state的有些字段是无法自由增删改查的，而我要让ai自由切换人设，上一句这个人，下一句模拟下一个人来与客户对话，需要state能够自由控制；
- langgraph的state的元数据比较复杂，没有用到就是制空；看过程日志好长是无效信息，看的有点累。
- 为什么把state拆分为节点间传输state和纯对话存储的state？
- 隔离不同类型的对话：节点间state只负责处理当前thread下的多轮对话，工具产生的信息只存在这里面（每轮对话开始前都要清空的）；纯对话state用于隔离杂乱信息，仅保存聊天信息和重要信息（不清空，永久保存到一条thread中）。
...
- 后续维护：
- 重构代码，完善功能，将langgraph全部替换为fastapi接口，
- 结合前后端，以桌面桌宠即时互动的形式展现。
